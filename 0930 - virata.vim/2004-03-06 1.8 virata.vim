...
" Maintainer:	Manuel M.H. Stol	<ManuelS@AlliedData.com>
" Version:	1.8
" Last Change:	2004-03-01
" Vim URL:	http://www.vim.org/
" Virata URL:	http://www.conexant.com/


" Virata AConfig Configuration Script Syntax:
" - This syntax is used in AConfig scripts to build ISOS firmware,
...
"     4) The subdirectory "/atmos/" somewhere in the scripts full path.
" - Note that due to mergers Virata has become part of Conexant.
...
syn match   virataStatement	"^\s*\(Architecture\|GetEnv\|Reserved\|\(Un\)\=Define\)\>" skipwhite nextgroup=@virataGrpIdents
syn match   virataStatement	"^\s*\(Hardware\|ModuleSource\|\(Release\)\=Path\|Software\)\>" skipwhite nextgroup=@virataGrpFileIdents
syn match   virataStatement	"^\s*ModuleVersion\>" skipwhite nextgroup=@virataGrpNumbers
syn match   virataStatement	"^\s*Requires\>" skipwhite nextgroup=@virataGrpIdents,@virataGrpNumbers
...
syn match   virataStatement	"^\s*NoWarn\s\+\(Duplicate\|Export\|Parse\=able\|Relative\)\>"
...
"  Defines
syn match   virataDefine	"^\s*\(Local\|\(Un\)\=Set\)\>" skipwhite nextgroup=@virataGrpIdents

...
" TODO:
"   ecb, cbc and/or something

let s:save_cpo = &cpo
set cpo&vim

" @param mixed key String or Byte List
" @param mixed data String or Byte List
" @param Number padding *0:None 1:PKCS#5 padding
" @return Byte List
function blowfish#encrypt(key, data, ...)
  let key = (type(a:key) == type("")) ? s:str2bytes(a:key) : a:key
  let data = (type(a:data) == type("")) ? s:str2bytes(a:data) : a:data
  let padding = get(a:000, 0, 0)
  let padding_byte = get(a:000, 0, 0x05)

  if len(key) < s:KEYBYTES && s:MAXKEYBYTES < len(key)
    throw printf("key length should be %d <= key <= %d", s:KEYBYTES, s:MAXKEYBYTES)
  endif

  if padding == 1
    let data = s:PKCS5_padding(data, s:BLOCKSIZE)
  endif

  if len(data) % s:BLOCKSIZE != 0
    throw "data not multiple of block length"
  endif

  let res = []

  let blowfish = deepcopy(s:Blowfish, 1)
  call blowfish.Initialize(key)
  for i in range(0, len(data) - 1, 8)
    let L = s:bitwise_lshift(data[i + 0], 24) + s:bitwise_lshift(data[i + 1], 16) + s:bitwise_lshift(data[i + 2], 8) + data[i + 3]
    let R = s:bitwise_lshift(data[i + 4], 24) + s:bitwise_lshift(data[i + 5], 16) + s:bitwise_lshift(data[i + 6], 8) + data[i + 7]
    let [L, R] = blowfish.encipher(L, R)
    call add(res, s:bitwise_rshift(L, 24))
    call add(res, s:bitwise_and(s:bitwise_rshift(L, 16), 0xFF))
    call add(res, s:bitwise_and(s:bitwise_rshift(L, 8), 0xFF))
    call add(res, s:bitwise_and(L, 0xFF))
    call add(res, s:bitwise_rshift(R, 24))
    call add(res, s:bitwise_and(s:bitwise_rshift(R, 16), 0xFF))
    call add(res, s:bitwise_and(s:bitwise_rshift(R, 8), 0xFF))
    call add(res, s:bitwise_and(R, 0xFF))
  endfor
...
" @param Number padding *0:None 1:PKCS#5 padding
" @return Byte List
function blowfish#decrypt(key, data, ...)
  let key = (type(a:key) == type("")) ? s:str2bytes(a:key) : a:key
  let data = (type(a:data) == type("")) ? s:str2bytes(a:data) : a:data
  let padding = get(a:000, 0, 0)
  ...
  " remove padding
  if padding == 1
    let res = s:PKCS5_unpadding(res, s:BLOCKSIZE)
  endif

  return res
endfunction

function blowfish#test()
  let password = s:str2bytes("MY PASSWORD")
  let data = s:str2bytes("The quick brown fox jumps over the lazy dog")

  " generated by c
  let expect = s:hex2bytes("2336495639a0e16a2f888f22d407df2ee0f80e1e7e625fe94b129fcd6ad37ad81f224cf58022ae617356d00f42869e27")
  let encrypted = blowfish#encrypt(password, data, 1)
  let decrypted = blowfish#decrypt(password, encrypted, 1)
  echo "encrypted:" s:bytes2hex(encrypted)
  echo "   expect:" s:bytes2hex(expect)
  echo "decrypted:" s:bytes2hex(decrypted)
  echo "     data:" s:bytes2hex(data)
  if encrypted == expect && decrypted == data
    echo "test: OK"
  else
    echo "test: NG"
  endif
  ...
  " openssl test
  " TODO: look into openssl manual or source
  let salt = s:str2bytes("MY SALT") " len(salt) <= 8
  if len(salt) < 8
  ...
  let t = system(printf('openssl bf-ecb -k "%s" -S "%s" | xxd -p', s:bytes2str(password), s:bytes2hex(salt)), s:bytes2str(data))
  let openssl_encrypted = s:hex2bytes(substitute(t, '\_s', '', 'g'))
  echo "    vim:" s:bytes2hex(encrypted)
  echo "openssl:" s:bytes2hex(openssl_encrypted)
  if encrypted == openssl_encrypted
    echo "openssl test: OK"
  else
    echo "openssl test: NG"
  endif
endfunction

function! s:PKCS5_padding(data, blocksize)
  " PKCS#5 padding
  let pslen = a:blocksize - (len(a:data) % a:blocksize)
  let ps = repeat([pslen], pslen)
  return a:data + ps
endfunction

function! s:PKCS5_unpadding(data, blocksize)
  " PKCS#5 padding
  let pslen = a:data[-1]
  if pslen < 1 || a:blocksize < pslen || pslen > len(a:data)
    throw "invalid padding length"
  endif
  let ps = a:data[-pslen : ]
  if ps != repeat([pslen], pslen)
    throw "invalid padding byte"
  endif
  return a:data[ : -pslen - 1]
endfunction

let s:MAXKEYBYTES  = 56   " 448 bits
let s:KEYBYTES = 8
let s:N = 16
let s:BLOCKSIZE = 8
...
function s:Blowfish.F(x)
  let x = a:x
  let d = s:bitwise_and(x, 0x00FF)
  let x = s:bitwise_rshift(x, 8)
  let c = s:bitwise_and(x, 0x00FF)
  let x = s:bitwise_rshift(x, 8)
  let b = s:bitwise_and(x, 0x00FF)
  let x = s:bitwise_rshift(x, 8)
  let a = s:bitwise_and(x, 0x00FF)
  let y = self.S[0][a] + self.S[1][b]
  let y = s:bitwise_xor(y, self.S[2][c])
  let y = y + self.S[3][d]
  return y
endfunction

" void Blowfish_encipher(unsigned long *xl, unsigned long *xr)
function s:Blowfish.encipher(xl, xr)
  let Xl = a:xl
  let Xr = a:xr
  for i in range(s:N)
    let Xl = s:bitwise_xor(Xl, self.P[i])
    let Xr = s:bitwise_xor(self.F(Xl), Xr)
    let [Xl, Xr] = [Xr, Xl]
  endfor
  let [Xl, Xr] = [Xr, Xl]
  let Xr = s:bitwise_xor(Xr, self.P[s:N])
  let Xl = s:bitwise_xor(Xl, self.P[s:N + 1])
  return [Xl, Xr]
endfunction

" void Blowfish_decipher(unsigned long *xl, unsigned long *xr)
function s:Blowfish.decipher(xl, xr)
  let Xl = a:xl
  let Xr = a:xr
  for i in range(s:N + 1, 2, -1)
    let Xl = s:bitwise_xor(Xl, self.P[i])
    let Xr = s:bitwise_xor(self.F(Xl), Xr)
    let [Xl, Xr] = [Xr, Xl]
  endfor
  let [Xl, Xr] = [Xr, Xl]
  let Xr = s:bitwise_xor(Xr, self.P[1])
  let Xl = s:bitwise_xor(Xl, self.P[0])
  return [Xl, Xr]
endfunction

" short InitializeBlowfish(char key[], short keybytes)
function s:Blowfish.Initialize(key)
  let self.P = deepcopy(s:P, 1)
  let self.S = deepcopy(s:S, 1)

  let j = 0
  for i in range(s:N + 2)
    let data = 0x00000000
    for k in range(4)
      let data = s:bitwise_or(s:bitwise_lshift(data, 8), a:key[j])
      let j = j + 1
      if j >= len(a:key)
        let j = 0
      endif
    endfor
    let self.P[i] = s:bitwise_xor(self.P[i], data)
  endfor

  let datal = 0x00000000
  let datar = 0x00000000

  for i in range(0, s:N + 1, 2)
    let [datal, datar] = self.encipher(datal, datar)
    let self.P[i] = datal
    let self.P[i + 1] = datar
  endfor

  for i in range(4)
    for j in range(0, 255, 2)
      let [datal, datar] = self.encipher(datal, datar)
      let self.S[i][j] = datal
      let self.S[i][j + 1] = datar
    endfor
  endfor
endfunction

...
"4. To close the search box hit <Esc>.
"
"Info on ctags file:
"-------------------
...
"Credits:
"--------
"The functions OnCursorMovedI and ExtractPromptedInput were adapted from Takeshi Nishida's fantastic fuzzyfinder.vim
"script (script_id=1984).
...
function s:OpenDrillWindow()
	...
	inoremap <buffer><silent> <Esc> :call <SID>ToggleDrill()<CR>
	inoremap <buffer><silent> <CR> <C-Y><Esc><C-W>gf<CR> :execute "bd!".bufnr("DrillResults")<CR>
	inoremap <buffer><silent> <expr> <BS> "\<C-E>\<BS>\<C-X><C-U>"
...
function! s:OnCursorMovedI()
    let input = s:ExtractPromptedInput(getline('1'))
    if line('.') != 1
        "Fix
        call feedkeys("\<Esc>" . input[1] . "\<CR>", 'n')
        return
    elseif !input[0]
...
function! s:ExtractPromptedInput(in)

...
fun! s:NetRead(...)
	...
	while ichoice <= a:0
		...
		if has("win32")
			...
			"Add by Hwang
			exe 'lcd ' . fnamemodify(tmpfile2,':h')
			let tmpfile2 = fnamemodify(tmpfile2,':t')
		endif
		...
		elseif b:netrw_method  == 2		" read with ftp + <.netrc>
			...
			"Add by Hwnag	
			if strlen(netrw_fname) != 0
				if strpart(netrw_fname, 0, 1) == "/" || strpart(netrw_fname, 0, 1) == "\\"
					exe "put ='ls ".netrw_fname." ".tmpfile2."'"
				else
					exe "put ='ls /".netrw_fname." ".tmpfile2."'"
				endif
			else
			...
			if f_isDirectory == 1
				...
				nnoremap <buffer> O  :call <SID>FtpFileOpen()<cr>
			endif
...
"Add by Hwnag
function! s:FtpFileOpen()

	if exists("g:netrw_port") && g:netrw_port != ""
		let portstr = ":" . g:netrw_port	
	else
		let portstr = ""
	endif
	...
	if strpart(lineStr, 0, 1) == "/" && strpart(lineStr, 0, 1) == "\\"
		let filename = "ftp://" . g:netrw_machine . g:netrw_port . lineStr
	else
		let filename = "ftp://" . g:netrw_machine . g:netrw_port . "/" . lineStr
	endif

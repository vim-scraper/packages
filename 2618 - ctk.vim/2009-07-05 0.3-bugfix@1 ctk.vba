...
ctk.vim makes you can easily and freely compiler your source code in Vim with
you own way. it read several modeline in your file to modifie the behavior
your command do, such as change command flags, add redirect when exec it, etc.
You can define your own hotkey to invoke ctk, and do any happily things with
it.
...
<
then, if you has a compiler_info line (|ctk-info-line|) in your cinfo file
...
:[N]CC[!] {entry}
		...
		|:ListCompiler|. {entry} is the command your invoke, in this
		...
		if it has a bang and don't has a entry, it use empty entry,
		that is, invoke "cmd" tag in your info line.if it doesn't has
		a bang, it will invoke the entry you used last time.
...
When you use ctk.vim. It will check all file you opend. if you open a new file
of which filetype has some info line in your cinfo file, it will be renamed by
ctk.vim. it only happend when g:ctk_autofname isn't empty |ctk_autofname|.
this option controls the way that ctk.vim product the new filename.

if a file has a autocreated name, this name will used in compile. when you
conpile it, it will be saved in a temporary folder |ctk_tempdir|. but the
executable output program will drop in your current folder (except when
current folder is $VIM, in this case program will drop to temporary folder).
the executable program will be named the value of g:ctk_defoutput
|ctk_defoutput|.
...
if you use tag += 'foobar', then foobar will add to the value of tag. e.g. if
the value of tag "flags" is "-Wall", "flags += '-mwindow'" will change flags
into '-Wall -mwindow'.

another example, if we didn't have tag named 'flags', "flags = '-Wall' will
...
where filetype is the filetype you want add compiler info in. compiler_name is
the name of this compiler, this name can use in modeline |ctk-modeline|. and
tags can use in modeline, its format is the same with in modeline.
|ctk-modeline-format|
...
"input" and "output". these tags used to create the command line to invoke
...
the $output in run tag is a child tag. it just used in command tag. you can
use any child tag you like. 

							       *ctk-entry-tag*

tags can be defined in several entry. if tagname isn't include the underline
(_), it will be a simple tag, a simple tag has a empty entry. e.g.  cmd, run,
input and output. if you just press :CC to invoke compiler. the empty entry
command tag "cmd" will be parsed and runed. if you just press :RUN to invoke
the program you compiled. the empty entry command tag "run" will be parsed adn
runed.

if you use any other words to invole compiler and program. e.g. use foobar.
the entry will be changed to foobar. if you press :CC foobar, the command tag
foobar_cmd will be parsed. if ctk can't find foobar_cmd in a info line, the
cmd will be parsed.

ctk use this turns to find the tags: first find it in info line, then find it
in default, if can't find, ctk find simple tag in info line, and in default.
...
for example (you can find this in the default cinfo file):
>
...
the first line tells ctk.txt c language file's extname is "c", some times
filetype name is the extname, but sometimes not. in this case, you can also
put this sentence into SetDefaultInfo: >
...
current info line (i.e. gcc), it will find the current default line. so you
can put some common infomations at here.

you needn't clear infomations (use SetCompilerInfo!) before you set it. if the
filetype of buffer is changed, all info line will be deleted. if you want
...
input and output. is says, if you want to compile a C program. the command
...
the asm_run is a entry tag |ctk-entry-tag|. you can use :RUN asm to invoke it.
in this case, all tags in this tag will be treat a entry tag. for asm_run,
":if bufname('^$output$') == '' | sp +drop $output | endif | checkt'", the
$output wil be treated as asm_output. if ctk can't find asm_output, it will
find output, and if it can't find output, it will use built-in value, since
it's a base tag. if the asm_run = '$foobar', and you doesn't support foobar
and asm_foobar in your info line, it will be remain as it is ($foobar).

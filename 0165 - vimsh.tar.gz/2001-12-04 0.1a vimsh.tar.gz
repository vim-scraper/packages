...
#  known issues/todo:
#  
#  TODO:  Make sure cmds are terminated with '\n'
...
#         when executed?
#  TODO:  Even after writing the prompt to the buffer the row
#         always seems to be returned as 0
...
#         I'm going down a line and continuing.  Tabs/spacing might be 
#         screwed up at the moment too.
#  TODO:  rm -i doesn't work, it might be due to trailing/leading spaces
#         I dunno yet.  The interactive prompt comes up but the file(s)
#         aren't deleted, rm by itself seems OK.
#
###############################################################################

import vim, sys, os, pty, tty, select, string
...
class vimsh:
    ...
    def execute_cmd( self ):
        ...
        if cur_line == self.prompt_line:
            ...
            exe_line = whole_line[ len( prompt ): ]

            print "executing " + exe_line
            self.write( exe_line + "\n" )
            self.read( cur )

            # TODO:  handle modified here

            vim.command("startinsert")

            ##  tuck away current cursor position for getting command
            self.prompt_line, self.prompt_cursor = vim.current.window.cursor
...
try:
    ##  TODO:  Need to come up with a way to generate these so

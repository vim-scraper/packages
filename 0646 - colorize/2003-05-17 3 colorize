...
def usage
  STDERR.puts "USAGE: #{ $0 } [--mode=gui|text] [--lang=vim_lang] [--strip-ws] [--cache [--cache-prefix=prefix]] [--headers] input-file [output-file]"
  exit 1
end
...
getopts(nil, "mode:text", "lang:__auto__", "headers", "strip-ws", "cache", "cache-prefix:/tmp/_colorize")

if ARGV.size != 2 and ARGV.size != 1
  usage
else

...
if !exists("g:CVSallowemptymsg")
  let g:CVSallowemptymsg = 'a'          " a:Add, i:Commit, j:Join in, p:Import
endif
...
let s:cvsmenucvs=":pserver:anonymous@ezytools.cvs.sourceforge.net:/cvsroot/ezytools"
...
" Menu entries		{{{1
"-----------------------------------------------------------------------------
" Spaces after items to inhibit translation (no one wants a 'cvs Differenz':)
...
function! CVSDoTag(usertag,tagopt)
  " force tagname input
  let tagname = CVSEscapeMessage(input('tagname: '))
  if tagname==''
...
function! CVSupdate()
  ...
  if s:CVSupdatequeryonly > 0
    ...
    unlet cvsoptbak oldkeyword removpat removidx mlen
  else
...
function! CVSadd()
  if (g:CVSusedefaultmsg =~ 'a') && ((g:CVSallowemptymsg =~ 'a') || (g:CVSdefaultmsg != ''))
    ...
    " force message input
    let message = CVSEscapeMessage(input('Message: '))
  endif
...
function! CVScommit()
  if (&modified || expand('%') == '') && (g:CVSforcedirectory == 0)
  ...
  if (g:CVSallowemptymsg !~ 'i') && (message == '')
...
function! CVSjoinin(...)
  ...
  elseif g:CVSusedefaultmsg =~ 'j' && g:CVSdefaultmsg != ''
    ...
    " force message input
    let message = CVSEscapeMessage(input('Message: '))
    if (g:CVSallowemptymsg !~ 'j') && (message == '')
...
function! CVSOpenLinks()
  ...
  if !filereadable(links)
    let @z = "\" ~/cvslinks.vim\n"
      ...
      \ . "call CVSGet('VimTools/cvsmenu.vim',':pserver:anonymous@ezytools.cvs.sourceforge.net:/cvsroot/ezytools','i','')\n"

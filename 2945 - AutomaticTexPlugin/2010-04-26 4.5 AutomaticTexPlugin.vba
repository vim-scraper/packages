...
ISearch()							*atp-ISearch()*
map \is, :IS
	Which makes an inverse search (sets the xdvi position according to the
...
<
    ...
    (2) Inverse searching
	For inverse searching (position xdvi according to the vim's cursor
	position) you can set:
>
	let b:inverse_search="xdvi -sourceposition " . line(".") . ":" . col(".") . fnamemodify(expand("%"),":p") . " " . fnamemodify(expand("%"),":p:r") . ".dvi"
<
...
>
 	...
 	let b:inverse_search="xdvi -editdor 'vim --servername " . v:servername
...
<
	...
	Inverse Searching:
	    If inverse searching do not works for you, and you get an error, that
	    ...
	    with path) in the b:inverse_search (as we do above using the
	    ...
	    b:inverse_search and in the file there is full path).

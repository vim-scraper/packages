...
3344
automatic-tex-plugin.txt 	For Vim version 7	Last change: 2010 September 14
...
map <LocalLeaader>np, map <LocalLeader>ns, map <LocalLeader>ns
	...
	Go to next <section>, :NSec goes to next section, chapter or part,
	other commands work in a similar way.
	...
	of |g:atp_mapNn|using the command |atp-:ToggleNn|.
...
<	will turn on the feature only for myfile.tex. Something more elaborate
	...
	list. 
								...
								*atp-:S*
:S[!] /{pattern}/ [flags]
	The E and S flags are removed. The pattern is a vim pattern (with
	'magic'). With bang "!" it regenerates the tree of input files.
...
<	will not.		
	...
	There is a new function to check where the input file is on the hard
	drive if the name in the input command doesn't include the full path.
	It asks kpsewhich which directories to search, filters out some

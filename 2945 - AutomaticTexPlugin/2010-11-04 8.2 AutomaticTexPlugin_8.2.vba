...
3637
automatic-tex-plugin.txt 	For Vim version 7	Last change: 4 November 2010
...
<
  ...
  - The default value of g:atp_vmap_environment_leader is <LocalLeader> (it
    ...
    <LocalLeader>E to wrap with environemnts: center, flushright, flushleft,
...
<
  ...
  - :AMSRef[!] {bib_data}  - a command to get bib data from amsref:
    'http://www.ams.org/mathscinet-mref'. The bib data will be appended to
    bibliography of the current document. If there are many bib files will get
    a choice which one to use. This command is also available in bib files.
    With bang "!" it writes and closes the bib buffer. You will be also asked
    for the bib key, the default choice is to use the AMS bib key.  This
    command also works when you use \begin{bibliography} environment. But the
    default is to use a bib file (which might be changed in the future).  The
    obtained bibliographic reference is copied to the register
    g:atp_bibrefRegister (the default is "0" so you can paste it using vim
    command 'p') and also is stored in the variable g:atp_bibref.
  ...
  - <Ctrl-j>, <Ctrl-k> motions redisigned: their purpose is to make it easy to
...
<   It is used be ViewOutput (|atp-:ViewOutput|), Compiler (|atp-:TEX|) and
  ...
  - MakeLatex : if aux or log files are loaded in a buffer then they will be
    ...
    wise vimgrep will search in the buffer not the hard diks version and they
    may differ). 
  - MakeLatex : returns with error message if log contains '! Emergency stop.'
  - vip : is not stoping inside a math zone (|g:atp_MathZones|).
...
<
  ...
  - :DefiSearch[!] [pattern] - the command repsects the options: 'ignorecase'
...
map }e
	...
	This function omits environments in comment lines.
							...
							*atp-}m* *atp-{M*
	{environment} can be also math or displayedmath. For convienience

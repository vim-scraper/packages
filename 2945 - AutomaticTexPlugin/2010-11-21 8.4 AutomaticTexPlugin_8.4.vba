...
3710
*automatic-tex-plugin.txt* 	For Vim version 7	Last change: 21 November 2010

			An Introduction to AUTOMATIC (La)TeX PLUGIN (ver 8.4)
...
<   This will only work for b:atp_LocalEnvironments, LaTeX and AMS
    ...
    Envrionment name abbreviations are defined only if there were not defined.
    So you can override the settings using |atprc| file (or |vimrc| file).

    Furthermore you can change the "=":	    
    							*g:atp_iabbrev_leader*
let g:atp_iabbrev_leader = "="
    It is used  in insert modde abbreviations like: =theorem=. 
...
<
  - If g:atp_map_forawrd_motion_leader = "}" then there noremap }} }.
...
<
  ...
  - :AMSRef[!] {bib_data}  - a command to get bib data from AMSRef:
    ...
    is also available in bib files.  With bang "!" it writes and closes the
    bib buffer. You will be also asked for the bib key, the default choice is
    to use the AMS bib key.  This command also works when you use
...
nmap gf
	...
	The current file is now shown with highlight group: |hl-WarningMsg|. 
							...
							*atp-:InputFiles*
:InputFiles				
	List input files of the current projetc. It also updates the
...
-----------------------------------------------------------------------------
COPY RIGHTS					*atp_bibtex-copy-rights*
	See |atp-copy-rights|.
doc/latexhelp.txt	[[[1
2430

...
variable 'g:atp_tab_completoin_modes'.
...
Completion works for:
	...
	environments
		...
		The algorithm tries to colose environment in many natural
...
g:atp_close_after_last_closed	= 1
				If set to one it will close opened environments after
				last closed environemnt:
>
 				\begin{theorem}
 					\begin{enumerate}
 					\item .....
 				...
 				\end{theorem}<Tab>
<
				will close enumerate just after \end{flushright},
				However, 	
>
 				\begin{theorem}
 					\begin{enumerate}
 				...
 				\end{theorem}<Tab>
<
				will put \end{minipage} below \begin{minipage}.
				Furthermore, if:
>
 				\begin{theorem}
 					\begin{enumerate}
 				...
 				\end{theorem}<Tab>
<
 				Then '\begin{minipage}' will be closed just after
				'\end{itemize}'.

				If this option is set to '0' then colosing is put in
				the line below the current line. The default is '1'.

				Furthermore, this option is in use only if the
				environments are in seprate lines, and
...
<
				...
				the first value	 - search for \(:\)  [ inline math ]
...
<
   					 (this checks if your are in some
					environments, for example inside
					\begin{tikzpicture} or \tikz{}, if 1 then the
					tikz commands will be available only inside
					this environments)
...
<
				...
				$ or $$ exluding \$, thus it will be one in any newly
				edited file!)

g:atp_math_modes		List of math modes.
g:atp_no_tab_map
...
environment mapps '[*', ']*') or 'g:atp_no_tab_map' (disables the tab mappsing
for completion, then comption is mapped to F7 but there is not mapping for
...
log analyzer mappings:				*atp-texlog* 
...
map \is, :RevSearch
	...
	Here I discribe how inverse/reverse searching is done. 
...
>
 	...
 	let b:reverse_search="xdvi -editdor 'vim --servername " . v:servername
	"' -sourceposition " . line(".") . ":" . col(".") . fnamemodify(expand("%"),":p") . " " . fnamemodify(expand("%"),":p:r") . ".dvi"
<

...
2840
automatic-tex-plugin.txt 	For Vim version 7	Last change: 2010 August 15
...
NEWS							*atp-news*
>
	Changes in version 7.3.4 and 7.4 
<				(in 7.4 I added :DeleteHistory[!] [file], see below.)
>
	:DefiSearch[!] [pattern]
...
>
	b:atp_running
<	This variable stor the current number of runing instances of latex.
...
<	a command which behaves like 'gf' was added. With the minor
	difference, it alsway gives the choice which input file to edit,
	regardless of the position of the cursor.

	The current file is now also shown, with highlight group: WarningMsg. 
...
<	which do the obvious thing (unless g:atp_nohistory = 1). DeleteHistory
	command deletes all entries of history variables for the file (default
	is current file), with bang "!" it deletes the whole history file.
	There is completion for file names present in the history database. It
	matches the file name (not the full path), use |c_CTRL-D| to list the
	matching files.

...
<	Copy the bibkey to register e,+ or paste directly to the buffer 
	in which :BibSearch was invoked, at last curosr position.	
...
to `<Tab>. 
	...
	Note: You can use <CTRL-T> in insert mode to insert shiwftwidth
	(see |i_CTRL-T|) and <CTRL-D> to delete one shiftwidht (read |i_CTRL-D|).

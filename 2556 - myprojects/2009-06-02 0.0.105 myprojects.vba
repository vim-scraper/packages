...
UseVimball
finish
plugin/myprojects.vim	[[[1
3273
"=============================================================================
" File:						myprojects.vim
" Author:					Frédéric Hardy - http://blog.mageekbox.net
" Date:						Tue Jun  2 12:44:12 CEST 2009
...
elseif !exists('myprojects_enable')
	...
	function s:openMyProjectsWindow()
		if !s:goToMyProjectsWindow()
			...
			if bufexists(g:myprojects_file)
				...
				nnoremap <silent> <buffer> <LocalLeader>b :call <SID>toggleProjectBuffers(<SID>getProjectName(line('.')), <SID>getProjectPath(line('.')), '')<CR>
	...
	function s:defineProject()
		...
		let myprojects[name]['attributes']['filter'] = s:inputFilter('Filter of project ''' . name . ''': ', attributes['filter'])
	...
	function s:create(line)
		...
		if indent >= 0
			try
				let project = s:defineProject()

				call s:echo('Create project ''' . project[0] . ''' from path ''' . project[1][project[0]]['attributes']['path'] . '''...')
				call s:put(s:buildMyProjects('', project[1][project[0]]['attributes']['path'], project[1], '', indent), a:line)
				call s:message('Project ''' . project[0] . ''' created.')
	...
	function s:edit(command, line)
		...
		silent execute 'nnoremap <buffer> <silent> <LocalLeader>b :call <SID>toggleProjectBuffers(''' . projectName. ''', ''' . projectPath . ''', '''')<CR>'
	...
	function s:svnCheckout(line)
		...
		if indent >= 0
			try
				...
				let svn = s:input('Svn reporitory of project ''' . project[0] . ''': ', '')
				...
				if svn == ''
					...
					let svnUser = s:input('Svn user of project ''' . project[0] . ''': ', '')

					if svnUser != ''
						let svnCommand .=  ' --username ' . shellescape(svnUser)

						let svnPassword = s:inputSecret('Svn password of project ''' . project[0] . ''': ', '')
					...
					let svnCommand .= ' ' . shellescape(project[1][project[0]]['attributes']['path'])

					try
						call s:echo('Do svn checkout of ''' . svn . ''' in ''' . project[1][project[0]]['attributes']['path'] . '''...')
						call s:svn(svnCommand)
						call s:message('Svn checkout of ''' . svn . ''' in ''' . project[1][project[0]]['attributes']['path'] . ''' done.')
						call s:echo('Create project ''' . project[0] . ''' from path ''' . project[1][project[0]]['attributes']['path'] . '''...')
						call s:put(s:buildMyProjects('', project[1][project[0]]['attributes']['path'], project[1], '', indent), a:line)
						call s:message('Project ''' . project[0] . ''' created.')
						call s:echo(s:getPath(a:line))
					catch /.*/
						call s:svnError('Unable to checkout ''' . svn . ''' in ''' . project[1][project[0]]['attributes']['path'] . ''':', v:exception)
					endtry
				endif
			catch /.*/
				call s:error(v:exception)
	...
	" Function s:toggleProjectBuffers() {{{2
	function s:toggleProjectBuffers(project, path, delete)
		if a:path != ''
			if !s:createBufferWindow('Buffers of project ''' . a:project . ''' in ''' . a:path . '''', a:project, a:path)
				...
				silent execute 'augroup ' . s:plugin
				silent execute 'au! BufWinLeave <buffer> au! ' . s:plugin . ' BufNew,BufDelete ' . a:path . '/*'

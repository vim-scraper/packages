"
"           Posting v1.31 - Stand-alone USENET/e-mail environment
"                         Target: GVim version 6.2
"                         Last Change: 2004 Jan 18
" 
...
" local environment, please have a look at the Settings folder below!
...
"
"					      *po_i_<M-<>*
" <M-<>	  		Apply this prior to composing if you want your
...
"			chronological order and removes quotes with
"			level greater than 2; text below remains
"			unchanged. Text written by yourself: Insert
...
"			<Bs>           go to previous non-empty line
...
"			<Esc>          quit
"
"			To remove elder quotes supply a [count] denoting
"			the maximum level allowed. (Please note, when
"			[count] is given <C-Cr> also checks the logical
"			structure, that's to say incoherent parts are
"			then removed thereby too even if their quoting
"			level might be within the given range!.) You may
"			issue this command from unquoted lines as well.
"			Scan mode then is turned on for the first
...
"			independent blocks of text. Hard line breaks
"			once entered will persist.) Lists which are not
...
"			soon as this place again becomes part of the
"			visible file area. Pressing it once means the
"			content of the window will move by a quarter of
"			its current height.
"
"					      *po_i_<S-PageUp>*
"					      *po_i_<S-PageDown>*
...
" 4. Invocation				      *posting-invocation*
"
" To ensure Posting gets involved automatically when loading a mail
" message file add
"
...
" time spent on start-up make use of Vim's built-in client-server
" facilities.
"
"	gvim --servername MAIL --remote-silent "+Post! 72"
"
" for instance would ensure a server which is already running will be used
" for composing. Shortcuts <M-z> (send) and <M-o> (postpone) then will
" have the meaning of "minimize" or "iconise" resp., in any case they
" won't force Vim to quit. (To ensure the server will terminate when
" leaving one might consider adding something like
"
"	gvim --servername MAIL --remote-send ":qa!<Cr>"
"
" to the local logout/shutdown script.)
"
" To avoid loading Posting at all, either remove it from your plugin
...
"
" Alternatively you may use Posting's "s:window_position" variable. (see
" Settings folder)
"
"------------------------------------------------------------------------
" 6. Highlighting			      *posting-highlighting*
...
" 7. Signature				      *posting-sigfile*
"
" Posting also may be used to automatically append signatures when
" composing. To do so simply create a list of signatures you like (each
" item has to be followed by a single number sign character ('#') located
" on a line of its own), and assign its path to "s:sig_file" (see below)
" so that Posting can find it. Then each time you press <M-p> or <M-z>
" resp. Posting will append one of those signatures to the message-body.
" The file being used may contain an arbitrary number of signatures.
"
"------------------------------------------------------------------------
" 8. Style				      *posting-quoting_style*
"
...
" 9. History				      *posting-history*
"
" 2004 Jan 18:	- Compose screen: Added support for top-posting (very
...
function! s:InstDoc()
  function! s:MakeDoc()
    ...
    1 d _ | 1/^"·/,$ d _
    % s/^\%("let \)\@!" \?//
    call setline(1, "*posting.txt*\tStand-alone USENET/e-mail environment (v1.31)")
...
function! s:Cr(form, mode, ...)
  ...
  if getline('.') =~ '^1>'
    ...
    norm! ms
    if search('^\x>', 'bW') > 0
      call s:TrimQuote((a:0 == 0 ? 2 : v:count), a:form)
      norm! `s
    endif
    call s:ConWin('update')
...
function! s:Interface(mode)
  ...
  elseif a:mode == 'compose'
    if s:guiSup
      ...
      anoremenu	  <silent> &Posting.&Quoting.A&ppend!<Tab>Alt+<		:call <SID>InitTopPost()<Cr>
      ...
      tm &Posting.&Quoting.A&ppend!					Generate top-posting style message
    ...
    ino	  <buffer> <silent> <M-<>		    <C-O>:call <SID>InitTopPost()<Cr>
...
if exists("s:helpDoc")
  echo "Posting v1.31: Installed help-documentation."
endif

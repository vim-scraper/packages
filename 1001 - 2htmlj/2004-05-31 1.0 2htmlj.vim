...
" Modified: 2004 May 31, by Charles Shooshan <chasshoo@snet.net> (CS)
" Primary Modifications:
"   1. html_use_css_id  Add the id attribute element identifer
"   2. short_name       Name the file by replacing final extension with html 
"   3. short_title      Set the html title to the source filename
"   4. javadoc          Sets all of the above for compatibility with javadoc
"   5. javadoc_no_lnr   Same as javadoc but does not display line numbers

" Transform a file into HTML, using the current syntax highlighting.

" Settings for javadoc and javadoc_no_lnr (CS)
if exists("javadoc")
  let html_use_css_id = 1
  let html_use_css = 1
...
" html_use_css_id must be used with html_use_css (CS)
if exists("html_use_css_id")
  let html_use_css = 1
endif
...
" Allow for short_title (CS)
if exists("short_title")
  exe "normal! a" . s:source_file . "</title>\n\e"
else
...
while s:lnum <= s:end
  ...
  " Allow for id attribute element identifer (id for each line set to line.{s.lnum}) (CS)
  if s:numblines
    if exists("html_use_css_id")
        let s:new = '<span id="line.' . s:lnum . '" class="lnr">' . strpart('        ', 0, strlen(line("$")) - strlen(s:lnum)) . s:lnum . '</span>  '
      else
        let s:new = '<span class="lnr">' . strpart('        ', 0, strlen(line("$")) - strlen(s:lnum)) . s:lnum . '</span>  '
    endif    
  else
    if exists("html_use_css_id")
      let s:new = '<span id="line.' . s:lnum . '"></span>'
    endif
  endif  

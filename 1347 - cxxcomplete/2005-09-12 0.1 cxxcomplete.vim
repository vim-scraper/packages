...
au FocusLost * :call OutWi()

let g:showing =0
let g:weHaveCompl=0

function! Rb_InitWindow()
ruby << BLOCK
require 'gtk2'

      wi = Gtk::Window.new(Gtk::Window::POPUP)
      $data=Array.new
      wi.window_position=(Gtk::Window::POS_CENTER)
      wi.border_width = 1
...
ruby << BLOCK
      ...
      row=VIM::evaluate("winline()")
      col=VIM::evaluate("wincol()")
      wi.move( col.to_i*10,row.to_i*20 )
      wi.show_all
      VIM::command("let g:showing=1")
      p treeview.get_column
BLOCK
endfunction
...
inoremap <esc> <c-r>=RealEscape()<cr>



function! ListUp()
ruby<<BLOCK
if path.prev! == true then
...
" classes. This may be faster than looking in tags but the
...
" Search tags for global variables?
...
" Complete all identifiers from tags?
...
if has("gui_running") || s:useWildMenu
	...
	tmenu &cppcomplete.&GenerateTags.&Rebuild\ from\ current\ directory Generate a new tags file from the files in the current dorectory
	...
	tmenu &cppcomplete.&GenerateTags.A&uto\ Generate\ a\ new\ one Auto generate a new tags file for C/C++.
	...
	tmenu &cppcomplete.&Use\ generated\ tag\ file\ in\ tags.&No Do not use tags as an ordinary tag file
	amenu &cppcomplete.&Use\ generated\ tag\ file\ in\ tags.&Yes<Tab>:InsertToTags   :InsertToTags<CR>
	tmenu &cppcomplete.&Use\ generated\ tag\ file\ in\ tags.&Yes Use tags as an ordinary tag file
	...
	tmenu &cppcomplete.&Toggle\ search\ options.Global\ &variables Search tags for global variables?
	...
	tmenu &cppcomplete.&Toggle\ search\ options.Inner\ class\ &names\ in\ tags Search tags for classes that is defined in other classes scope?
	...
	tmenu &cppcomplete.&Preview\ menu.Scan\ for\ new\ &items Scan tags for classes, structures and unions
...
function! s:InsertToTags()
	if (match(&tags, "tags,",0)>=0)
		call confirm("tags is already in tags","&OK",1,"Info")
	else
		let &tags="tags," . &tags
	endif
endfunction
...
function! s:ClearFromTags()
	if (match(&tags,"tags")<0)
		call confirm("tags did not include tags","&OK",1,"Info")
	else
		let &tags=substitute(&tags,"tags.","","g")
	endif
endfunction
...
function! s:RefreshMenu()	
	...
	let spaceAfter="[^!\t]\\+\t"
	let res=confirm("If you have a big tags file may strange things happen", "&All\n&Just items in the current directory\n&Cancel",2,"Warning")
	if res==1
	...
	if s:useBuffer && s:neverUseGrep
		...
		quit
	elseif s:useDJGPP
		call s:SetGrepArg("'^" . spaceAfter . fileSelect . spaceAfter . s:groupS . "c\\|s\\|u\\)' tags")
		silent! let items=system(s:grepPrg . " @" . s:grepTemp)
	else
		let items=system(s:grepPrg . " '^" . spaceAfter . fileSelect . spaceAfter . s:groupS . "c\\|s\\|u\\)' tags")
	endif
	let nextM=0
	let nclines=0
	let nslines=0
...
function ICompleteNarrowList( )
"      if strlen( currentline) >3 genMenu( or use a cache)
		call s:BuildMenu()
            if g:weHaveCompl
                  call Rb_Refresh()
            else
...
" sets up insert mode mapping for all letter keys
function! ComplBndgs()
	" initialize buffer local variables
	let b:icomplete_line = 0
	let b:icomplete_col  = 0

		let chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_"
		let start = strlen(chars)
		while start > 0
			let var = chars[start-1]
			exe "inoremap <silent> <buffer> " var var . "<C-o>:silent! call ICompleteNarrowList()<cr>"
                  			let start = start - 1
                  
		endwhile

		inoremap <silent> <buffer> . .<c-o>:silent! call ICompleteNarrowList()<cr>
		inoremap <silent> <buffer> > ><c-o>:silent! call ICompleteNarrowList()<cr>
		inoremap <silent> <buffer> : :<c-o>:silent! call ICompleteNarrowList()<cr>
"		inoremap <silent> <buffer> ( (<c-r>=ICompleteListCompletions()<cr>
"		inoremap <silent> <buffer> <bs> <bs><c-o>:silent! call ICompleteNarrowList()<cr>
endfun
...
function! s:BuildIt()
	...
	if has("gui_running") || s:useWildMenu
		aunmenu PopUp
            ruby $data=[]
"            ruby $data.push("notfound_2")
	endif
	if (s:matches=="")
		if has("gui_running") || s:useWildMenu
			amenu PopUp.no\ completions\ found_1*   :let @c=''<CR>
                  let g:weHaveCompl=0
                  ruby $data=[]
"                  ruby $data.push("notfound_3")
		endif
		return
	endif
      let g:weHaveCompl=1
	...
	let pMore=""
	let totHits=0
" where all the stuff are stuffed inside PopUp
	while (s:tooBig>s:nHits) && (match(s:matches,"\t",nextM)>0)

"     	call confirm("From the original ", "&OK", 1, "Info")
...
ruby <<BLOCK
line=VIM::evaluate("pMore . @c . accStr")
#line.sub(/< Tab>/,  '\t')
$data.push(line.sub(/<Tab>/," : \t"))
BLOCK
			endif
	...
	if (s:nHits>=s:tooBig)
		...
		if has("gui_running") || s:useWildMenu 
			if !s:useWinMenu
				...
				amenu PopUp.Max\ number\ of\ hits\ reached* <NOP>
			endif
		endif
...
function! s:UpdateInheritList()
	...
	if (s:listAge!=getftime("tags"))
		if s:useBuffer && s:neverUseGrep
			...
			quit
		elseif s:useDJGPP
			call s:SetGrepArg("'^" . after3 . ".*" ."inherits:.*' tags")
			silent! let s:inheritsList="\n" . system(s:grepPrg . " @" . s:grepTemp)
		else
			let s:inheritsList="\n" . system(s:grepPrg . " '^" . after3 . ".*" ."inherits:.*' tags")
		endif
		let s:listAge=getftime("tags")
	endif
endfunction
...
function! s:BuildFromOrdinary()
	...
	elseif s:useDJGPP
		...
		let s:matches=system(s:grepPrg . " @" . s:grepTemp . " tags")
	else
		let s:matches=system(s:grepPrg . maxGrep . " '^" . s:uTyped . "[^ \t]*\t' tags")
	endif
endfunction
...
function! s:BuildMenu()
	...
	if &previewwindow
		call confirm("Can't do this is \"Preview window\"","&OK",1,"Error")
		return
	endif
	let s:nHits=0
	if ! s:CheckForTagFile()
	...
	if (s:gotCType)
		...
		if s:colonSep
			...
			elseif (s:useDJGPP)
				call s:SetGrepArg("'^" . s:groupS . firstPart . s:classList . secondPart . firstPart . s:clType . "\t\\)' tags")
				silent! let s:matches=system(s:grepPrg . " @" . s:grepTemp)
			else
				let s:matches=system(s:grepPrg . maxGrep . " '^" . s:groupS . firstPart . s:classList . secondPart . firstPart . s:clType . "\t\\)' tags")
			endif
			...
			elseif s:useDJGPP
				...
				silent! let s:matches=system(s:grepPrg . " @" . s:grepTemp . " tags")
			else
				let s:matches=system(s:grepPrg . maxGrep . " '^" . firstPart . s:classList . secondPart . "' tags")
			endif
		endif
...
function! s:CheckClassType()
	...
	if s:useBuffer 
		...
		quit
	elseif s:useDJGPP
		call s:SetGrepArg("'^" . s:clType . "\t" . spaceAfter . spaceAfter . goodTypes . "' tags")
		silent! let foundIt=system(s:grepPrg . " @" . s:grepTemp)
	else
		let foundIt=system(s:grepPrg . " '^" . s:clType . "\t" . spaceAfter . spaceAfter . goodTypes . "' tags")
	endif
	if foundIt==""
		let s:gotCType=0
...
function! s:BuildBlock(regex)
	...
	if s:useBuffer
		...
		quit
	elseif s:useDJGPP
		call s:SetGrepArg("'" . a:regex ."' tags")
		let s:regexBlock=system(s:grepPrg . " @" . s:grepTemp . " | " . s:grepPrg . " '^[^!]'")
	else
		let s:regexBlock=system(s:grepPrg . " '" . a:regex ."' tags | " . s:grepPrg ." '^[^!]'")
	endif
	if s:regexBlock==""
...
function! s:CheckHiddenLoaded()
	if (!bufexists("tags")) || (getftime("tags")!=s:bufAge)
		silent! execute "edit! tags"
		let &buflisted=0
		let &bufhidden="hide"
		let &swapfile=0
		let s:bufAge=getftime("tags")
...
function! s:DoGlobalJump()
	...
	if s:useBuffer 
		...
		quit
	elseif s:useDJGPP
		call s:SetGrepArg("'^" . searchFor . "\t" . spaceAfter . spaceAfter . tStr . "' tags")
		silent! let foundIt=system(s:grepPrg . " @" . s:grepTemp)
	else
		let foundIt=system(s:grepPrg . " '^" . searchFor . "\t" . spaceAfter . spaceAfter . tStr . "' tags")
	endif
	if foundIt!=""
...
function! s:CheckForTagFile()
	if s:nannyMode && s:HasNotAsked()
		return s:NannyCheck()
	elseif getftime("tags")==-1
		if s:nannyMode 
			return s:NannyCheck()
		endif
		call confirm("No tags found!","&OK",1,"Error")
		return 0
	endif
	return 1
endfunction
...
function! s:NannyCheck()
	if getftime("tags")==-1
		if s:currLanguage=="Java"
			let ans=confirm("No tags file!\n\nGenerate from:", "Files in this dir\n&Cancel",1,"Question")
			if ans==1
	...
	elseif s:currLanguage=="Java"
		let ans=confirm("A tags file already exists.","&Use\n&Generate new\nC&omplete it\n&Cancel",1,"Question")
		if ans==2
			call s:GenerateTags()
		elseif ans==3
...
function! s:GetIncludeFiles()
	...
	if missedIncFiles>0
		let str1="Of " . totalIncFiles . " files was " . missedIncFiles . " not found."
            let str1=missedIncFiles. " files not found\n from a total of ". totalIncFiles
		...
		echon "\r                                                           "
		echon "\rcould findd all files... generating tags"
	endif
	return 1
endfunction
...
function! s:GenerateFromCheckpath()
	if getftime("tags")!=-1
		let dial=confirm("You have a tags file!\n\nYou want to replace it?", "&Yes\n&No",2,"Warning")
		if (dial!=1)
			return 0
		endif
		call delete("tags")
	...
	if has("win32") || has("win16") || has("dos16") || has("dos32")
		...
		call system("ctags -n --language-force=C++ -f tags --fields=+ai --C++-types=+p -L " . s:ctagsTemp)
	else
		call system("ctags -n --language-force=C++ -f tags --fields=+ai --C++-types=+p " . s:includedFiles)
	endif
	call s:UpdatehiddenBuffer()
	return 1
endfunction
...
function! s:AppendFromCheckpath()
	...
	if has("win32") || has("win16") || has("dos16") || has("dos32")
		call s:AnotherLongCommandLinePatchForWindows()
		call system("ctags --append -n  --language-force=C++  -f tags --fields=+ai --C++-types=+p " . s:ctagsTemp)
	else
		call system("ctags -n -a --language-force=C++ -f tags --fields=+ai --C++-types=+p " . s:includedFiles)
	endif
	call s:UpdatehiddenBuffer()
	return 1
endfunction
...
function! s:BrowseNFiles()
	call s:CheckFiletype() 
	let browseFile=""
	let browseFile=browse(0, "File to include in tags","./","")
	if (browseFile!="")
		if (s:currLanguage=="Java")
			call system("ctags -n -a -f tags --fields=+ai " . browseFile)
		else
			call system("ctags -n -a -f tags --fields=+ai --C++-types=+p " . browseFile)
		endif
		call s:UpdatehiddenBuffer()
	endif
endfunction
...
function! s:GenerateAndAppend()
	call s:CheckFiletype() 
	if (s:currLanguage=="Java")
		call system("ctags -n -a -f tags --fields=+ai *")
	else
		call system("ctags -n -a -f tags --fields=+ai --C++-types=+p *")
	endif
	call s:UpdatehiddenBuffer()
	return 1
endfunction
...
function! s:GenerateTags()
	call s:CheckFiletype() 
	if getftime("tags")!=-1
		let dial=confirm("You already have a tags!\n\nYou want to replace it?", "&Yes\n&No",2,"Warning")
		if (dial!=1)
			return 0
		endif
		call delete("tags")
	...
	if (s:currLanguage=="Java")
		execute "!ctags -n -f tags --fields=+ai *"
	else
		execute "!ctags -n -f tags --fields=+ai --C++-types=+p *"
	endif
	call s:UpdatehiddenBuffer()
	return 1
endfunction
